...
- T2. 用户认证与资料模块：已完成，commit = `a6d7416`，说明：新增登录前邮箱验证门禁，确保开启验证时未验证账号不能登录，相关 Jest 回归测试已通过。
- T3. 队伍与成员管理：已完成，commit = `5f954e9`，说明：审批通过时增加队伍已满/重复加入校验，确保队伍人数与成员唯一性不被破坏。
- T4. 匹配推荐：已完成，commit = `139f0b8`，说明：在未传入 skills 时保留默认排序并返回推荐列表，避免筛选条件为空导致推荐结果被误过滤。
- T5. 任务协作：已完成，commit = `5c4a6ab`，说明：禁止非法状态转移，限制 `CONFIRMED` 仅队长可执行，并补齐任务状态回归测试。
